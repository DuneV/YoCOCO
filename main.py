...
from transaction import download_video_only, download_video
...
YOLO_MODEL   = os.getenv("YOLO_MODEL", "models/yolov8s-e.pt")
...
MAX_DUR       = int(os.getenv("MAX_DUR_SEC", "1200"))
...
def filter_frame(
    ...
    for h in hands:
        ...
        # Objeto más cercano al index tip
        closest_obj, dist = find_closest_object(ix, iy, dets)

        if closest_obj is None:
            continue

        touch = is_touching(dist)
        grasp = is_grasping((wx,wy), (ix,iy), closest_obj["bbox"])
        grasp_prob = grasp_probability((wx,wy), (ix,iy), closest_obj["bbox"])

        out["interactions"].append({
            "hand_label": h.get("label"),
            "closest_object": closest_obj.get("label"),
            "distance": dist,
            "touching": touch,
            "grasping": grasp,
...
def process_one(url: str, enriched_info: Optional[dict] = None):
    ...
    path = download_video(url, OUT_DIR) if WITH_AUDIO else download_video_only(url, OUT_DIR)
    ...
    def last_t(arr): return (arr[-1].get("time_sec") if arr else 0) or 0
    ...
    if USE_DB:
        with db.session() as s:
            va = VideoAnalysis(
                ...
                arms_pose=mp_clean,
                summary={"source": "youtube", "url": url, "notes": "YOLO+MediaPipe aligned; cleaned"},
                meta={
                    "yolo": {"model": YOLO_MODEL, "conf": YOLO_CONF, "iou": YOLO_IOU, "imgsz": YOLO_IMGSZ, "batch": YOLO_BATCH},
                    "mediapipe": {"max_hands": MP_MAX_HANDS, "det_conf": MP_DET_CONF, "trk_conf": MP_TRK_CONF, "fps_out": MP_FPS_OUT},
                    "filters": {
...
def collect_ids_one_attempt(queries, days_back, max_items_per_query, duration, order, region, lang):
    """Recolecta IDs sin desperdiciar, respetando límites."""
    published_after_iso = (
    ...
    print(f"[IDS] {len(dedup)} IDs recolectados en esta variante.")
    return dedup
